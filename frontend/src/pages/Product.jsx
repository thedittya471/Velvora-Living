  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        ...
        setImage(response?.data?.data?.images[0]?.url ?? null)
      } catch (err) {
        ...
        <div className='flex-1 flex flex-col-reverse gap-3 sm:flex-row'>
          <div className='flex sm:flex-col overflow-x-auto sm:overscroll-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full'>
            {(productData.images || []).map((item, index) => (
              <img onClick={()=>setImage(item.url)} src={item.url} key={index} className='w-[24%] sm:w-full sm:mb-3 shrink-0 cursor-pointer' alt='' />
            ))}
          </div>
